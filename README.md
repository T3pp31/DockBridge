...
- Public releases are unsigned; on first launch, right-click `DockBridge.app` in Applications and choose **Open** (or allow it under **System Settings → Privacy & Security**). Developer ID signing and notarization are planned (see [#391](https://github.com/T3pp31/DockBridge/issues/391)); until then, verify downloads with the published SHA-256 checksum and `gh attestation verify`.
