...
impl TransferManager {
    ...
    pub async fn enqueue_upload_entry(
            ...
            .map_err(transfer_error_from_sftp)?
        {
            ...
            let tasks: Vec<TransferTask> = files
                .iter()
                .map(|entry| {
            ...
            // Batch directory creation once, before any file upload.
            ...
            for task in tasks {
                ...
                let final_task = match self.run_upload_task(session, task).await {
                    ...
                    Err(_) => {
                        batch.failed += 1;
                        executed.push(self.find_task(task_id).unwrap_or_else(|| TransferTask {
                            id: task_id,
...
mod tests {
    ...
    async fn enqueue_upload_entry_continues_after_one_file_failure() {
        ...
        // Remote: the two successful files exist under the dir-name-nested root.
