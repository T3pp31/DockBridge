...
impl<'a> SftpClient<'a> {
    ...
    /// Deletes an empty remote directory. Rejects attempts to delete the
    /// file system root to avoid destructive path confusion.
    pub async fn delete_directory(&self, remote_path: &str) -> Result<(), SftpError> {
        let remote_path = normalize_remote_path(remote_path)?;
        if remote_path == "/" {
            return Err(SftpError::DeleteFailed {
                path: remote_path,
                message: "cannot delete the file system root".to_string(),
            });
        }
        self.sftp()
